...
def load_image_align_data(dest_path, image_paths, image_size, margin, pnet, rnet, onet, discarded_folder_path='',
   ...
   logger.info('path of cropped_big_face:' + cropped_big_face)
   ...
   for img_path, x in zip(image_files, range(len(image_list))):
      ...
      if nrof_samples > 0:
         ...
         if big_face_no < 0:
            ...
            logger.debug('confidence score of ' + img_name + " is: " + str(round(bounding_boxes[big_face_no][4], 3)))  # print in log: confidence score of big face detected and localized.
            cropped, bb = crop_image_by_bbox(image_list[x], det, img_size, margin)
