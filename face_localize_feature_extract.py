...
logging.basicConfig(filename= "facenet_logfile.log", level = logging.DEBUG, format='%(asctime)s - %(message)s ', datefmt='%m/%d/%Y %I:%M:%S %p')
    ...
    for img_path,x in zip(image_files,range(len(image_list))):
        ...
        if nrof_samples > 0:
            ...
            if big_face_no < 0:
                ...
                logging.info('confidence score of ' + img_name + " is: " + str(round(bounding_boxes[big_face_no][4],3))) #print in log: confidence score of big face detected and localized.
                cropped,bb = crop_image_by_bbox(image_list[x], det, img_size)
...
print("Path of output folder is: ", dest_path)
